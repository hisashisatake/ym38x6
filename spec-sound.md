...
- AR/D1R/D2R/RR・フィードバック・KSR・AM感度・PM感度は、実機OPM/OPN（ymfm実装）の
  理論値を両端のアンカーとした指数カーブとする（実機の離散的なカーブをそのまま
  再現するのではなく、よりなだらかな変化にする）。詳細は各パラメーターの備考を参照
...
| RR（リリースレート） | 4bit | 0〜255 | 指数カーブ（8.71ms〜284.9秒、0は実機にない「リリースしない」拡張） |
...
- nih-plugの制約により、MIDI Program ChangeイベントはCLAPでのみ受信可能（VST3では
...
- 音色は`Preset { name: String, patch: Ym38x6Patch }`としてJSON化し、`.38x6`拡張子で保存する
  フォーマットを`ym38x6-core`に定義した。現在は`Ym38x6Patch`のフィールドのみが対象で、
  拡張波形スロット(8〜255、未実装)のデータ埋め込みは当該機能実装後に別途対応する。
- 読み込み元ディレクトリは`%APPDATA%\ym38x6\presets`が存在すればそちらを使い、
  ...
  `b<bank>_p<program>.38x6`（ファイル名でバンク/プログラム番号を指定、命名規則は暫定）を
  DAWロード時に読み込み、Program Change受信時に該当ファイルがあればその音色を、
  なければ`placeholder_patch`を使う。
- プリセットの保存（書き出し）UI・操作は未実装（フェーズ5「パラメーターUI」で対応予定）。
  `Preset::to_json`は今後gesture-app等から呼び出して書き出す想定。
