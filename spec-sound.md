...
**実装状況（フェーズ5着手時点）：**
- CC0+CC32によるバンク選択とProgram Changeの受信を実装済み。Bank0/Bank1以降の
  実音色データ（ym38x6-ml生成・プリセットライブラリ）ができるまでは、bank・program番号から
  決定的に生成する暫定パッチ（`placeholder_patch`）を両バンク共通で使用する。
