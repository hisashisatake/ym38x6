...
| 項目 | 制御 | 備考 |
|------|-----|------|
| Rate | CC76（Vibrato Rate） | |
| Depth | CC77（ベース値）+ CC1（加算分） | Destinationにより単位・モデルが異なる（下記） |
| Delay | CC78（Vibrato Delay） | キーオンから効果開始までの遅延 |
| 波形 | NRPN「Performance LFO Waveform」で選択（下記） | デフォルト = 三角波 |
| Destination | NRPN「Performance LFO Destination」で選択（下記） | デフォルト = F-Number（ビブラート） |
...
| 値 | 宛先 | Depthのモデル |
|---|---|---|
| 0（デフォルト） | F-Number（ビブラート） | `実効Depth(セント) = CC77値 + CC1値 × RPN0,5値 / 127` をF-Numberに加算 |
| 1 | TL（全オペレーター一括、トレモロ） | `実効Depth = clamp(CC77値 + CC1値, 0, 255)` を各ノートの実効TL（Velocity Sensitivity適用後）に加算 |
| 2 | TL（キャリア一括、トレモロ） | 同上（キャリアのみ） |

トレモロ（Destination=1/2）は各ノートの実効TLに対して相対的に作用するため、ベロシティによる音量差は維持されたまま揺れる。
RPN 0,5（Modulation Depth Range）はDestination=F-Numberの場合のみ意味を持つ（詳細はRPNセクション参照）。
...
**チャンネル単位（13個）：**
Feedback / パフォーマンスLFO Rate / パフォーマンスLFO Depth（ベース値）/ パフォーマンスLFO Delay / Filter Cutoff / Filter Resonance / Filter EG A / Filter EG D / Filter EG S / Filter EG R / Filter EG Depth / Reverb Send / Chorus Send
...
| 0, 5 | Modulation Depth Range | 64（約50セント相当） | パフォーマンスLFO Destination=F-Numberの場合のCC1セント換算係数（パフォーマンスLFOセクション参照） |
...
フェーズ3: OP単位F-Number・独立キーオンを実装
  → OPQ由来の音楽的表現を一般化して活用

フェーズ4: パラメーターUI・音色保存・プリセットライブラリ
  → PSR-70音色データのコンバートも含む
  → GM2プログラムマップ準拠のBank 0音色セット作成
  → Bank Select / Program Change 実装
  → マスターエフェクト（Reverb / Chorus）実装
...
フェーズ8: MLによる音色生成（ym38x6-ml、オプション）
